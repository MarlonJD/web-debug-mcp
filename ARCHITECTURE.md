...
5. `web_issue_capture` returns evidence schema 3 with DOM, console, network, screenshot, paused-frame, and optional React/Angular/Vue bridge data, then applies core redaction again. Checks-only snapshots omit every framework enrichment; paused snapshots use explicit last-known/stale page-runtime caches.
