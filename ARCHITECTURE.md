...
| `src/adapters/interactive-elements.ts` | Shared bounded DOM discovery and locator selection for the explicit interactive UI capture surface | Platform Engineering; update with the cross-transport discovery contract |
| `src/adapters/webmcp.ts` | Bounded page-provided WebMCP discovery/execution contract used only by `ChromiumAdapter` | Platform Engineering; update with the WebMCP page API contract |
...
| `plugins/web-debug/skills/webmcp-tool-authoring/SKILL.md` | Minimal authority-gated WebMCP product authoring and mutation-evidence guidance | Platform Engineering; update with WebMCP authoring policy changes |
...
`src/index.ts` is the only MCP tool boundary. It validates inputs and each tool's advertised output with Zod, delegates to `SessionManager`, and returns one canonical structured envelope plus a bounded text preview. It maps scenario attempts to MCP progress notifications and registers opaque screenshot resources through `ArtifactStore`; it does not access Playwright directly. Package, npm/GitHub release, bundled MCP, and plugin runtime share final identity `0.10.0`. Clients that register a local checkout under a distinct name must not enable both registrations in one session.
...
The plugin's `manual-parity-qualification` and `webmcp-tool-authoring` skills are orchestration and guidance layers outside `SessionManager`. They can derive candidate requirements, require explicit review, map approved cases to repository-native tests, and validate non-executable qualification metadata. They do not add an MCP tool, reuse adaptive bug-reproduction retries for business mutations, perform arbitrary authenticated read-backs, persist MCP scenarios, or treat Web Debug diagnostics as qualification PASS. The target repository owns actors, setup/reset, native Playwright/API/domain execution, and durable artifacts; WebMCP production registration requires approved product authority.
...
Page-runtime intelligence is selected from private detected-framework metadata. Chromium registers separate target-scoped React, Angular, and Vue bridge scripts before navigation, stores every CDP identifier, removes every identifier on close, and snapshots only requested, detected runtimes concurrently under one optional-enrichment budget. Manual summary captures skip optional framework, accessibility, interactive-element, and WebMCP collectors; base browser observations remain available for summary and replay. Verification representatives use full collection independently of the manual profile. React retains bounded DevTools-hook commit/render evidence. Angular uses documented development `window.ng` globals to build a DOM-host component/state view without private Ivy arrays, getters, methods, signals, injectors, or profiler mutation. Vue 3 safely chains the exact DevTools hook event contract and never falls back to DOM-private `__vue*` properties. Vite and Next HTTP bodies remain bounded. Angular CLI's encapsulated Vite server is not the Web Debug Vite endpoint; only actual Vite projects with `webDebugVitePlugin()` receive module/HMR provenance. The selected `interactiveElements` surface is shared DOM discovery: Chromium live-validates semantic/test-id/label/text locators, while Safari emits stable CSS locators only. Nullable or unavailable framework fields remain warnings, not discovery failures.

The target-scoped `WebMcpPageApi` is optional and page-provided. It reports only `webmcp-page-api` provenance, discovers without cross-origin expansion, executes one explicitly authorized direct action with a validated JSON string and opaque string/null result, and never contributes an executable replay/scenario action. A completed or attempted direct call marks the session replay timeline non-restorable and suppresses subsequent screenshots.
...
2. `web_session_start` allocates a session ID and artifact directory, starts the selected adapter, then records separate `projectCapabilities` and live negotiated `runtimeCapabilities` for Chromium launch/attach or Safari WebDriver/BiDi. Chromium reports the page API only as `webmcp-page-api` provenance.
3. Browser actions are bounded and same-origin. Replayable actions carry navigate, click, fill, press, select, check, hover, scroll, observable wait, and reload behavior; a separate direct union adds one WebMCP action without making it scenario/replay executable. Console, request, response, and page-error observers retain bounded metadata in memory.
...
5. `web_issue_capture` returns capture schema 6: compact `summary` by default, explicit `full`, selected `include`, or cursor-based `delta`. The per-surface `collection` map distinguishes fresh, stale, skipped, undetected, unavailable, and privacy-suppressed evidence. Cursors hash only collected surfaces and their states; they do not merge older cursor knowledge. The optional `interactiveElements` surface is a bounded visible/action map with locators, live uniqueness, enabled/checked state, and geometry; it never includes control values and does not serve as a product assertion. WebMCP metadata is discover-only and visibly untrusted. Screenshot pixels require explicit full/inclusion, but every capture after a direct WebMCP attempt is suppressed; local paths never enter capture data. Private scenario verification continues to use authoritative evidence schema 4; checks-only snapshots omit framework/interactive enrichment and paused snapshots use explicit last-known/stale caches.
6. If the project has Vite capability, `ViteAdapter` queries the local `__web_debug/vite` endpoint when Vite evidence is selected (or during full verification) and adds its bounded module graph/HMR metadata to the browser evidence.
7. If the project has Next capability, `NextAdapter` queries the local `/_next/mcp` endpoint when Next evidence is selected (or during full verification) and adds its bounded runtime metadata to the browser evidence.
...
9. After each replayable browser action and evidence capture, `SessionManager` stores a bounded, sanitised replay frame; `web_replay_seek` returns one retained frame and can replay only safe retained actions when `restore: true`. A direct WebMCP attempt appends only an `action: null` frame, permanently marks the timeline non-restorable, and suppresses later screenshots.
