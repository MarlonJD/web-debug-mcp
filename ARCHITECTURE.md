...
`src/index.ts` is the only MCP tool boundary. It validates inputs and each tool's advertised output with Zod, delegates to `SessionManager`, and returns one canonical structured envelope plus a bounded text preview. It maps scenario attempts to MCP progress notifications and registers opaque screenshot resources through `ArtifactStore`; it does not access Playwright directly. Local source identity is `0.7.0-next.0`; the immutable released npm/GitHub and installed plugin runtime remain `0.6.0`. Clients that register a local checkout under a distinct name must not enable both registrations in one session.
...
5. `web_issue_capture` returns capture schema 5: compact `summary` by default, explicit `full`, selected `include`, or cursor-based `delta`. WebMCP metadata is discover-only and visibly untrusted. Screenshot pixels require explicit full/inclusion, but every capture after a direct WebMCP attempt is suppressed; local paths never enter capture data. Private scenario verification continues to use authoritative evidence schema 5; checks-only snapshots omit framework enrichment and paused snapshots use explicit last-known/stale caches.
