...
- `interactiveElements` is a selected capture surface for agent-assisted UI exploration. It reports only bounded visible candidates, existing locators, live match counts, enabled/checked state, and geometry; it never proves a product requirement. Chromium validates semantic locators with `probe`; Safari keeps the surface CSS-only and may omit candidates without a stable `id` or `data-testid` selector.
