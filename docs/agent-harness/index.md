...
The repository adopts the full HMAC-backed harness certification procedure. Release `0.4.0` refreshes the source/direct-child evidence window with the owner-only key outside the repository; the formal verifier returns `CERT000` for bounded repository-local `harness-ready` integrity. The native gate reports `fresh-structure-candidate` because it checks structure, ancestry, hash, expiry, and worktree state but intentionally does not authenticate the HMAC. An approved external CDP host and provider-backed production authority remain separate and are not inferred from local smoke output.
