...
| Live React/Vite smoke | `npm run smoke:react-vite` | Exercise injected React component/state bridge, Vite source URL/module graph, breakpoint, verification, and cleanup | JSON reports `passed: true` with React, Vite, and breakpoint assertions | Platform Engineering; React/Vite adapter change | verified locally |
