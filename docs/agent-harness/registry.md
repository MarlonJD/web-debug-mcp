...
| Live Safari smoke | `npm run smoke:safari` | Exercise Safari WebDriver actions, DOM, screenshot, cleanup, and explicit unsupported-capability warnings | JSON reports `passed: true`; current host is `blocked` until Safari remote automation is enabled | Requires macOS Safari remote automation or a loopback WebDriver endpoint | Platform Engineering; Safari adapter or browser policy change | blocked |
