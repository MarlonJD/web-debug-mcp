...
`web_issue_capture` returns capture schema 6 under `data`. Its default summary is capped at 16 KiB and produces no pixels; explicit full, selected include, and cursor-based delta profiles expose bounded detail. Summary skips optional framework/HTTP/accessibility/WebMCP enrichment. Included and delta surfaces drive collection, while private scenario representatives remain full. Cursor digests contain only that capture’s collected surfaces and their state; newly requested surfaces are changed relative to a cursor that skipped them. The result carries:
