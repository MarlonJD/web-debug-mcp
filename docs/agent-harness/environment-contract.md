...
After building, run `node bin/web-debug-mcp.mjs doctor` with the exact project/browser options to check readiness without launching a browser. Doctor stdout is one bounded JSON report with separate registry and `client-binding` checks. The client-binding check is always `warn`: doctor cannot inspect the active Codex task's tool namespace, and only a real bundled `web_project_detect` call proves Web Debug Gate 0. If that call is unavailable, stop with `MCP_CLIENT_BINDING_UNAVAILABLE` or `MCP_SERVER_STARTUP_UNAVAILABLE`, use Settings → MCP servers → Restart (or a new task/session), and do not substitute a browser runner or a naked MCP process.
