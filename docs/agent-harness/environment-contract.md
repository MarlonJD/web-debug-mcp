...
The optional Codex/ChatGPT/Claude Code plugin is installed from the repository marketplace. Codex uses `.agents/plugins/marketplace.json`; Claude Code uses `.claude-plugin/marketplace.json`. Both point to the same local stdio MCP server through `plugins/web-debug/.mcp.json`, pinned to immutable public npm release `web-debug-mcp@0.6.0`; enabling the plugin does not create a hosted endpoint or a second browser controller. The repository source-next identity is `0.7.0-next.0` and is not installed by the plugin. The first server start uses `npx` to resolve the npm package, so Node.js 20+, npm, and network access are required at first launch.
