...
| React/Vite adapter | `npm run typecheck` | `npm run smoke:react-vite` | Component tree, hook state, source breakpoint, Vite module graph/HMR status, screenshot, and scenario verification | Requires Vite dependencies and an explicit Chromium executable | Platform Engineering; React bridge, Vite plugin, or fixture change |
| Next adapter | `npm run typecheck` | `npm run smoke:next` | Next runtime tools, routes, project metadata, compilation issues, route handler, and browser evidence | Requires Next dev server and an explicit Chromium executable | Platform Engineering; Next adapter or fixture change |
