...
Authoritative issue evidence remains schema version 4. Public manual capture is schema version 6 and uses a profile wrapper: `summary` by default, explicit `full`, selected `include`, or current changed surfaces through a reusable session-bound `delta` cursor. The capture `collection` map reports fresh, stale, not-collected, not-detected, unavailable, or suppressed state per surface. Summary skips optional enrichment, including `interactiveElements`; explicit surfaces select collectors. The selected `interactiveElements` surface is a bounded visible/action map with live locator counts, enabled/checked state, and geometry, never control values or product assertions. Verification remains full. Skipped collectors do not become negative findings or known cursor digests. Capture may discover bounded WebMCP metadata only; it never executes tools and labels metadata `untrusted: true`. Summary produces no screenshot; full and explicit screenshot inclusion opt into pixels, while auth/private-input suppression and post-WebMCP-attempt suppression always win. Local screenshot paths never cross the capture wire boundary. Angular/Vue enrichment remains Chromium development-only, checks-only attempts return null fields, paused captures are explicitly stale, and Safari remains generic WebDriver evidence.

Project detection schema 2 reports project kind, confidence, provenance, ambiguity, weak candidate signals, bounded declared-workspace candidates, and `projectCapabilities`. A live session summary schema 3 separately reports adapter-negotiated `runtimeCapabilities`; Chromium's WebMCP capability is truthful page-API (`webmcp-page-api`) provenance rather than native-browser provenance. The selected `interactiveElements` capture surface is DOM-derived and transport-aware: Chromium can emit live-validated semantic locators, while Safari emits stable CSS locators only. Project dependencies never claim that Safari exposes a JavaScript debugger, semantic locator, accessibility, TLS/auth, matrix, or WebMCP surface.
