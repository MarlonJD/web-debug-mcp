...
- Adds the bounded `doctor` CLI, full 13-tool in-memory routing coverage, source/test type checking, compatibility evidence, and process-group smoke teardown.
...
Verified locally with 106 deterministic tests, source/test typecheck, build, plugin validation, final tarball/fresh-prefix stdio handshake, Chromium/React-Vite/Next/Safari/local-fidelity smokes, and the six-scenario comparison demo. This is local release evidence, not provider-backed production attestation.
