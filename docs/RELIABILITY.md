...
| Safari remote automation is disabled or WebDriver is unavailable | `SafariAdapter` returns a bounded startup error; the smoke reports `status: "blocked"`; current macOS smoke is verified | Enable Safari remote automation or provide a loopback WebDriver endpoint; use Chromium when CDP debugger evidence is required | `safari-adapter.test.ts` and `npm run smoke:safari` |
