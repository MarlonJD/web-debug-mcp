...
This matrix records final `0.6.0` behavior on exact local development fixtures and package handshakes. A version not listed as verified is not implicitly unsupported; it is candidate-only until its deterministic contract and live smoke pass. Exact local observations are recorded in [`compatibility-evidence.json`](compatibility-evidence.json); they are not HMAC certification, provider authority, deployment, or production evidence.
...
| Safari | Local W3C WebDriver | Safari `26.6.2` deterministic WebDriver/BiDi and negotiated-capability contracts pass; immutable `0.5.0` retains historical live DOM/action/screenshot evidence | Fresh `0.6.0` live capture is blocked after two fixture-wait timeouts on this host; debugger parity and pre-request top-level interception remain unavailable |
