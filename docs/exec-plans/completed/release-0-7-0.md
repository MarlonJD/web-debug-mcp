...
- [x] (2026-08-31 15:10Z) Promoted every live package/plugin/marketplace/document/test/verification identity to final `0.7.0` and added release notes.
- [x] (2026-08-31 15:21Z) Passed deterministic, type/build, production audit, skill/plugin, all final live-browser/comparison, and cleanup gates; exact-archive Node 20/22/24 gates follow the clean release commit.
...
- The three newly completed plans were checked for ordered sections, UTC progress/revision entries, registry state/title, and semantic-review attestations before release freeze.
...
Current source and installed local plugin are `0.7.0-next.0`; immutable public npm/GitHub/remote marketplace remain `0.6.0`. Release surfaces include package/lock, live docs, release identity tests, verification contracts, both plugin manifests and marketplaces, bundled `.mcp.json`, release notes, compatibility evidence, and installed Codex plugin state. Historical completed ExecPlans are not rewritten.
...
1. Promote final package/plugin/marketplace/docs/tests/verification identities and run focused gates.
2. Run full deterministic, type/build, audit, deterministic checks, skill/plugin, and live-browser gates.
...
- `npm test`, `npm run typecheck`, `npm run build`, `npm test`, `git diff --check`, production audit, three skill validators, and plugin validator pass.
...
- Final verification: 32 files/161 tests, typecheck/build, production audit zero vulnerabilities, three skill validators, plugin validator, all live browser/framework/Safari WebDriver smokes, and one-run comparison.
