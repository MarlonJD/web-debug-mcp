...
- Preserve `v0.3.0`, `v0.3.1`, and `v0.3.2`; create only the new exact `v0.3.3` correction tag/release.
...
- [x] Publish npm `web-debug-mcp@0.3.2` and complete the initial fresh-prefix 13-tool package/stdio handshake; the handshake exposed stale server metadata.
- [ ] Publish npm correction `web-debug-mcp@0.3.3` and complete a fresh-prefix package/stdio handshake with matching server metadata.
- [ ] Refresh the `web-debug` Codex marketplace and install/verify the corrected enabled plugin build without a duplicate standalone MCP registration.
...
- Decision: Release `0.3.2` as a patch version, then release `0.3.3` as a forward-only metadata correction after fresh-prefix verification found the MCP server still advertising `0.3.1`. Rationale: npm versions and tags are immutable, so the bad artifact is preserved and the corrected artifact gets a new patch version.

## Validation and Acceptance

Acceptance requires `npm test`, `npm run typecheck`, `npm run build`, `npm run harness:check`, the skill validator, warning-free `npm pack`, a fresh-prefix package/binary MCP handshake listing all tools with `serverInfo.version: 0.3.3`, exact public npm metadata for `0.3.3`, source/main/tag/GitHub-release SHA equality, and Codex reporting `web-debug@web-debug` installed and enabled at the timestamped `0.3.3+codex.*` build. The skill must explicitly distinguish Web Debug, Build Web Apps, and native deterministic runners, and must state that an explicit Web Debug request cannot be silently substituted.
...
The routing implementation is committed and pushed as `d3dfb738e43c199e3fbd8acb3798fa8b7f5c4c77`; exact tag `v0.3.2` and its GitHub release point to that source revision. Public `0.3.2` packaging was verified with 13 tools but exposed stale `serverInfo.version: 0.3.1`; the correction is now prepared as `0.3.3`. Codex installation remains pending until the correction release is published and verified.
...
- (2026-08-28 12:33Z) Published npm `0.3.2` after browser authorization, then fresh-prefix handshake found stale MCP server version metadata; prepared forward-only `0.3.3` correction without moving `v0.3.2`.
